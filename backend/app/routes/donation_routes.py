...
def add_donation():
    if session.get('perms', {}).get('insert') != 'Y':
        flash('Unauthorized', 'danger')
        return redirect(url_for('donation_bp.list_donations'))
    form = DonationForm()
    if form.validate_on_submit():
        new_donation = Donation(**form.data)
...
def edit_donation(donationID):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('donation_bp.view_donation', donationID=donation.donationID))
    return render_template('donation_form.html', form=form)
